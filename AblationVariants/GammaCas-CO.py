...
class OutputLayer (tf.keras.layers.Layer):
  ...
  def call(self, A, Lambda, gamma, P, step=None):
    if step is None:
      ...
      out = tf.squeeze(integrate.odeint(IF, tf.zeros_like(A), P))
...
class NewModel (tf.keras.models.Model):
  ...
  def call(self, C, P):
    C = self.norm1(C)
    l_out = self.lstm(C)
    A = tf.expand_dims(self.ff_A(l_out), axis=-1)
    Lambda = tf.expand_dims(self.ff_lambda(l_out), axis=-1)
    Gamma = tf.expand_dims(self.ff_gamma(l_out), axis=-1)
    
    ret_count = self.autoreg(A, Lambda, Gamma, None, self.step)
    final_count = self.aggregate(A, Lambda, Gamma, P)
    return ret_count, final_count

  def loss(self, C, P, y2, lossType):
    Obs_count = C[:, :-1, :]
    y1 = C[:, 1:, 0]
    y1_, y2_ = self.call(Obs_count, P)
    mse = tf.keras.losses.mean_squared_error
    mape = tf.keras.losses.mean_absolute_percentage_error
    return tf.reduce_mean(mse(y_true=y1, y_pred=y1_) + mape(y_true=y2, y_pred=y2_))
    
  def grad(self, inputs, deltaPreds, final_size, lossType_):
    with tf.GradientTape() as tape:
      loss_value = self.loss(inputs, deltaPreds, final_size, lossType_)
...
ckpt = tf.train.Checkpoint(model=model,
                           optimizer=optimizer)
checkpoint_path = "models/softplus_updated.ckpt"
...
for epoch in range(num_epochs):
  ...
  for x, y in zip(trainDataset, trainDatasetY):
      ...
      loss_value, grads = model.grad(x, p1, y, loss_)
      optimizer.apply_gradients(zip(grads, model.trainable_variables))

      train_loss.update_state(loss_value)

  for x, y in zip(testDataset, testDatasetY):
      ...
      loss_value = model.loss(x, p1, y, loss_)
